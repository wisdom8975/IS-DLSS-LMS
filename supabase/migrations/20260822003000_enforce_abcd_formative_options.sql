-- Formative assessments use exactly four choices: A, B, C and D.
...
as $$
declare
  v_module_id uuid; v_course_id uuid; v_score numeric:=0; v_max numeric:=0; q record;
  selected text; selected_norm text; correct_norm text; option_value text;
  v_attempt_id uuid; v_completed_at timestamptz; v_count integer:=0; v_option_index integer;
begin
  ...
  for q in select qq.id,qq.correct_answer,qq.options,qq.points from public.quiz_questions qq where qq.lesson_id=p_lesson_id order by qq.position loop
    v_count:=v_count+1; v_max:=v_max+coalesce(q.points,1); selected:=p_answers->>q.id::text;
    selected_norm:=lower(trim(coalesce(selected,''))); correct_norm:=lower(trim(coalesce(q.correct_answer,'')));
    if selected_norm<>'' then
      if selected_norm=correct_norm then v_score:=v_score+coalesce(q.points,1);
        ...
        elsif jsonb_typeof(q.options)='object' then option_value:=q.options->>upper(selected_norm); end if;

-- Fix lesson formative RPC return ambiguity and keep server-side marking authoritative.
-- The RPC returns JSON instead of RETURNS TABLE so completed_at cannot collide with an output parameter.
...
begin
  ...
  if not exists(
    ...
    where ce.course_id=v_course_id and ce.student_id=auth.uid() and ce.active
  ) then
    raise exception 'Student is not enrolled in this course';
  end if;

  for q in
    select qq.id,qq.correct_answer,qq.options,qq.points
  ...
  loop
    ...
    if selected_norm<>'' then
      ...
      elsif selected_norm~'^[a-d]$' then
        if jsonb_typeof(q.options)='array' then
          option_value:=q.options->>(ascii(upper(selected_norm))-65);
        elsif jsonb_typeof(q.options)='object' then
          option_value:=q.options->>upper(selected_norm);
        end if;
        if lower(trim(coalesce(option_value,'')))=correct_norm then
          v_score:=v_score+coalesce(q.points,1);
        end if;
      end if;
    end if;
  ...
  if v_count<>20 then
    raise exception 'This lesson does not have exactly 20 assessment questions configured';
  end if;
  ...
  select lfa.id,lfa.completed_at into v_attempt_id,v_completed_at
  from public.lesson_formative_attempts lfa
  where lfa.lesson_id=p_lesson_id and lfa.student_id=auth.uid();

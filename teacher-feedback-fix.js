...
(function(){
  ...
  async function openFeedback(attemptId){
    ...
    const {data:attempt,error:attemptError}=await window.sb.from('quiz_attempts').select('id,student_id,module_id,score,max_score,submitted_at').eq('id',attemptId).single();
    ...
    if(window.role==='Teacher' && course.instructor_id!==window.currentUser.id) throw new Error('You are not authorized to review this assessment.');
    if(window.role==='Student' && attempt.student_id!==window.currentUser.id) throw new Error('You are not authorized to view this assessment.');
    const {data:existing,error:feedbackError}=await window.sb.from('assessment_feedback').select('id,reviewer_id,feedback,strengths,improvement_area,action_plan,updated_at').eq('attempt_id',attemptId).maybeSingle();
    ...
    overlay.innerHTML=`<div style="background:#fff;width:min(760px,100%);max-height:92vh;overflow:auto;border-radius:18px;padding:20px;box-shadow:0 24px 80px rgba(0,0,0,.28)">
      ...
      <div class="notice" style="margin:14px 0">${readOnly?'Teacher feedback and your action plan are shown below.':'Live updates are paused while you type. Nothing is saved until you press Save Feedback.'}</div>
    ...
    if(!readOnly){
      ...
      saveButton.onclick=async function(){
        ...
        status.textContent='';
        try{
          const payload={attempt_id:attemptId,reviewer_id:window.currentUser.id,feedback:document.getElementById('isd_fb_feedback').value.trim(),strengths:document.getElementById('isd_fb_strengths').value.trim(),improvement_area:document.getElementById('isd_fb_improvement').value.trim(),action_plan:document.getElementById('isd_fb_action').value.trim()};
          ...
          if(existing?.id){const {error}=await window.sb.from('assessment_feedback').update(payload).eq('id',existing.id);if(error) throw error;}
          else{const {error}=await window.sb.from('assessment_feedback').insert(payload);if(error) throw error;}
  ...
  function install(){
    ...
    window.assessmentDetail=async function(attemptId){try{await openFeedback(attemptId);}catch(error){window.__isdFeedbackEditing=false;alert(error.message||'Unable to open assessment feedback.');}};
    window.__isdTeacherFeedbackFix=true;
  }
  install();
  const timer=setInterval(function(){install();if(window.__isdTeacherFeedbackFix) clearInterval(timer);},100);
})();

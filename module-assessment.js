...
(function () {
  ...
  function key(moduleId) {
    const user = getUser();
    return 'isdlss_module_assessment_v2:' + (user?.id || 'anonymous') + ':' + moduleId;
  }
  ...
  function readDraft() {
    try { return JSON.parse(localStorage.getItem(key(state.moduleId)) || 'null'); } catch (_) { return null; }
  }
  function saveDraft() {
    try {
  ...
  function clearDraft() { try { localStorage.removeItem(key(state.moduleId)); } catch (_) {} }

  function normalizeQuestions(rows) {
    if (!Array.isArray(rows)) return [];
    return rows.map(function (q, position) {
  ...
  function render() {
    ...
    if (!root || state.submitted) return;
    ...
    root.innerHTML = `
      <section class="card" data-module-assessment="${escapeAttr(VERSION)}">
        <div class="kpi"><div><span class="badge">MODULE FORMATIVE ASSESSMENT</span><h3 style="margin:7px 0 3px">Question ${state.index + 1} of ${TOTAL_QUESTIONS}</h3><div class="small muted">${totalAnswered} of ${TOTAL_QUESTIONS} answered</div></div><strong>${percent}%</strong></div>
        ...
        <div class="choices" role="radiogroup" aria-label="Answer choices">
          ${q.options.map(function (option, i) {
            ...
            return `<button type="button" class="choice ${selected ? 'selected' : ''}" data-option="${escapeAttr(option)}" aria-pressed="${selected ? 'true' : 'false'}"><b>${letters[i]}.</b> ${escapeHtml(option)}</button>`;
          }).join('')}
        </div>
        ...
        <div class="actions" style="justify-content:space-between;margin-top:14px">
          ...
          ${state.index < TOTAL_QUESTIONS - 1 ? '<button type="button" class="btn" data-action="next">Next →</button>' : '<button type="button" class="btn" data-action="submit">Submit Assessment</button>'}
        </div>
    ...
    if (previous) previous.addEventListener('click', function () { state.index = Math.max(0, state.index - 1); saveDraft(); render(); });
  ...
  async function fetchQuestions(moduleId) {
    ...
    return questions.map(function (q) { return { ...q, options: shuffleOptions(q.options) }; });
  }
  ...
  async function submitAssessment() {
    ...
    try {
      if (!navigator.onLine) throw new Error('You are offline. Reconnect to the internet before submitting the assessment. Your 20 answers remain safely saved on this device.');
      const payload = { module_id: state.moduleId, student_id: user.id, answers: state.answers, client_submission_id: submissionId };
      const { data, error } = await client.from('quiz_attempts').insert(payload).select('id,module_id,student_id,score,max_score,submitted_at,client_submission_id').single();
      if (error) {
        const duplicate = error.code === '23505' || /duplicate|unique/i.test(error.message || '');
        if (duplicate) {
          const existing = await client.from('quiz_attempts').select('id,module_id,student_id,score,max_score,submitted_at,client_submission_id').eq('client_submission_id', submissionId).maybeSingle();
          if (existing.error) throw existing.error;
          if (existing.data) { clearDraft(); state.submitted = true; await showResult(existing.data, true); return; }
        }
        throw error;
      }
      clearDraft();
      ...
      const msg = root.querySelector('#moduleAssessmentMessage');
      if (msg) msg.innerHTML = '<div class="dangerbox">' + escapeHtml(message) + '</div>';
    }
  }
  ...
  async function showResult(attempt, duplicate) {
    ...
    if (attempt?.id && client) {
      const response = await client.from('assessment_feedback').select('feedback,strengths,improvement_area,action_plan,updated_at').eq('attempt_id', attempt.id).maybeSingle();
    ...
    root.innerHTML = `
      <section class="card" data-module-assessment-result="${escapeAttr(VERSION)}">
        ...
        <div class="feedback ${percent >= 50 ? 'good' : 'bad'}"><b>Score: ${Number(attempt.score || 0)}/${Number(attempt.max_score || TOTAL_QUESTIONS)} (${percent}%)</b><br>${percent >= 50 ? 'Good work. Your server-verified result has been recorded.' : 'Review the module content and use the feedback below to guide your next attempt.'}</div>
        ${feedback ? `<div class="card" style="margin-top:12px"><h4>Teacher feedback</h4><p>${escapeHtml(feedback.feedback || '')}</p>${feedback.strengths ? `<p><b>Strengths:</b> ${escapeHtml(feedback.strengths)}</p>` : ''}${feedback.improvement_area ? `<p><b>Improvement area:</b> ${escapeHtml(feedback.improvement_area)}</p>` : ''}${feedback.action_plan ? `<div class="notice"><b>Action plan:</b> ${escapeHtml(feedback.action_plan)}</div>` : ''}</div>` : '<div class="notice">Teacher feedback will appear here after your teacher reviews this assessment.</div>'}
  ...
  function boot() { autoAttach(); observer.observe(document.body, { childList: true, subtree: true }); }

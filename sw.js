const CACHE='isdlss-v20260822-9';
...
self.addEventListener('fetch',event=>{
  ...
  if(req.mode==='navigate'||url.pathname.endsWith('/index.html')){
    event.respondWith(fetch(req,{cache:'no-store'}).then(async response=>{
      ...
      const stamp='20260822-20q-v9';
      const loader='<script src="assessment-loader.js?v='+stamp+'" defer data-assessment-loader></script><script src="lesson-formative.js?v='+stamp+'" defer data-lesson-formative></script>';
      const injected=text.includes('data-assessment-loader')?text:text.replace('</head>',loader+'</head>');

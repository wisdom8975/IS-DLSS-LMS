...
// Production-finalizer is augmented with post-auth and navigation stability patches.
const LOGIN_FIX='/login-stability-fix.js';
const ASSESSMENT_FIX='/assessment-stability-fix.js';
const NAVIGATION_FIX='/navigation-stability-fix.js';
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith((async()=>{
    try{
      ...
      const [loginFix,assessmentFix,navigationFix]=await Promise.all([
        ...
        fetch(new URL(NAVIGATION_FIX,self.location.origin),{cache:'no-store'}).then(r=>r.ok?r.text():'').catch(()=>'' )
      ]);
      return new Response(source+'\n'+loginFix+'\n'+assessmentFix+'\n'+navigationFix,{status:original.status,statusText:original.statusText,headers:{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-store'}});

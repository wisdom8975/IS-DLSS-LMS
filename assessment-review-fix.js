...
(function(){
  ...
  function installFeedbackRefreshGuard(){
    ...
    if(typeof window.refreshLiveSurface!=='function') return;
    const original=window.refreshLiveSurface;
    window.refreshLiveSurface=async function(reason){
      if(feedbackEditorOpen()) return;
      return original.apply(this,arguments);
    };
    window.__isdFeedbackRefreshGuardInstalled=true;
  }

  installFeedbackRefreshGuard();
  const guardTimer=setInterval(function(){
    installFeedbackRefreshGuard();
    if(window.__isdFeedbackRefreshGuardInstalled) clearInterval(guardTimer);
  },100);
})();
